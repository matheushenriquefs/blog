export default class IntlService {
  ...
  getLocaleDate(date, locale = "en", options = null) {
    return new Intl.DateTimeFormat(locale, options ? options : {}).format(
      new Date(`${date} 00:00`)
    );
  }
  ...
  getLocaleDateOptions(date, options) {
    ...
    const isSameYear =
      currentDate.getUTCFullYear() === postDate.getUTCFullYear();
  ...
  getLocalePostDate(date, locale, options) {
    return this.getLocaleDate(
      date,
      locale,
      this.getLocaleDateOptions(date, options)
    );
  }
}
